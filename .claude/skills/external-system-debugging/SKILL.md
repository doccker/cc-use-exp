---
name: external-system-debugging
...
|------|------|-------------|
| `superpowers:systematic-debugging` | 通用 bug 调试 | 上游通用方法论;本 skill 在其之上加"先抓数据"硬规则 |
| `api-design-safety` | 自己设计 API 时 | 主动设计;本 skill 是被动适配第三方 |
...
- [代理关键词子串匹配误判](references/case-proxy-keyword-misjudge.md) — 正常长文章含 Cloudflare 关键词被代理误判为伪成功错误,强/弱特征拆分修复
...
```
> 📋 本回复遵循:`external-system-debugging` - [章节]
```
