...
> **部署位置**: `~/.claude/rules/ops-safety.md`
> **生效范围**: 始终生效
...
本规范定义了在执行系统级命令时的安全要求。目的是防止因不完整的操作导致系统故障。
...
```
> 📋 本回复遵循规则：`ops-safety.md` - [具体章节]
```

示例：
```
> 📋 本回复遵循规则：`ops-safety.md` - 系统级命令规则
> 📋 本回复遵循规则：`ops-safety.md` - 回滚方案要求
```
