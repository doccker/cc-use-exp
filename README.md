...
[![version](https://img.shields.io/badge/version-1.0.22-blue.svg)](https://github.com/doccker/cc-use-exp)
...
| Cursor | `.cursor/` | 项目内 `.cursor/rules/` + 用户级 `~/.cursor/skills/`/`~/.cursor/templates/` | ✅ 完整支持（项目内 rules + 增量兼容同步） |
...
| 项目内 | 部署目标 | 方式 |
|--------|---------|------|
| `.cursor/rules/*.mdc` | `~/.cursor/rules/` | 兼容性同步，manifest 管理；项目内 `.cursor/rules/` 仍是主路径 |
| `.cursor/skills/*` | `~/.cursor/skills/` | 增量复制，manifest 管理 |
| `.cursor/commands/*.md` | `~/.cursor/skills/{name}/SKILL.md` | 单文件 → 目录，作为 `/命令` 兼容层 |
...
- [CONTRIBUTING.md](./CONTRIBUTING.md) - 贡献流程与授权条款
- [CLA.md](./CLA.md) - 单独签署的贡献授权协议模板
- [TRADEMARK.md](./TRADEMARK.md) - 项目名称、Logo 与品牌使用规则
- [docs/licensing/historical-rights-checklist.md](./docs/licensing/historical-rights-checklist.md) - 历史贡献清权清单
...
本项目采用 `PolyForm Noncommercial 1.0.0` 发布；公开许可证仅覆盖非商业用途。任何商业使用、企业内部生产使用、面向客户交付或托管服务，均应事先获得维护者的单独商业授权。

| 用途 | 条款 |
|------|------|
| **个人学习、研究、实验、业余项目** | 通常可在非商业前提下使用、修改和分发 |
| **学校、公益组织、公共研究机构使用** | 通常可在许可证允许范围内使用 |
| **商业主体使用、企业内部生产使用、团队日常业务使用** | 默认不在公开许可证授权范围内，需单独获得商业授权 |
| **面向客户交付、代开发、SaaS 托管、付费咨询或付费培训** | 不在公开许可证授权范围内，需单独获得商业授权 |
| **转载/二次开发后再分发** | 仅限非商业场景，且需附带许可证文本或其 URL，并保留项目提供的 `NOTICE` |

这是一份 `source-available` 非商用许可证，不属于 OSI 定义下的开源许可证。

商业授权咨询：`doccker@pm.me`

详见 [LICENSE](./LICENSE)

项目附带的 [NOTICE](./NOTICE) 用于保留版权、来源和商业授权联系信息。

项目名称、Logo、封面图和其他品牌元素不随源码许可自动授权，详见 [TRADEMARK.md](./TRADEMARK.md)。
...
| 公司内部评估、内部工具、团队日常使用 | 通常应按商业用途处理，先联系维护者获取商业授权 |
| 面向客户交付、代开发、SaaS 托管、付费咨询或付费培训 | 应按商业用途处理，先联系维护者获取商业授权 |
| 二次开发后公开发布 | 仅当你的使用和再分发仍属非商业用途时，才可按本许可证进行，并需附带许可证文本或其 URL，且保留 `NOTICE` |
| 想将本项目集成进商业产品、商业服务或企业工作流 | 请先联系维护者取得书面商业授权 |
| 直接拷入第三方 copyleft 代码 | 不建议。`GPL`/`AGPL`/`LGPL`/`MPL` 等材料可能削弱维护者的商业再授权能力，应先做专项兼容性审查 |
| 拿不准是否属于商业用途 | 不要自行假设，先联系维护者确认 |

> 说明：这里的 FAQ 是项目维护者对当前许可策略的使用指引，不替代正式法律意见。

> 本项目当前通过 [CONTRIBUTING.md](./CONTRIBUTING.md) 约定默认贡献条款，并提供 [CLA.md](./CLA.md) 作为可单独签署的正式授权模板。对非琐碎外部贡献，维护者可在合并前要求先签署 `CLA`。
