...
> 详见 `async-task-pattern` skill - 陷阱 #5
...
前端遇到接口 30s 超时、用户重复点击、loading 转圈卡死，根因常在后端是同步接口。**不要在前端加 timeout 重试**，让后端改成 trigger + polling 模式（见 `async-task-pattern` skill）。前端配合：
